class ZCL_ZSD_ORDER_DPC definition
  public
...
protected section.
  ...
  methods HT001WSET_CREATE_ENTITY
    ...
    exporting
      !ER_ENTITY type ZCL_ZSD_ORDER_MPC=>TS_HT001W
    raising
  ...
  methods HT001WSET_GET_ENTITYSET
    ...
    exporting
      !ET_ENTITYSET type ZCL_ZSD_ORDER_MPC=>TT_HT001W
  ...
  methods VBAPSET_CREATE_ENTITY
    importing
      ...
      !IO_DATA_PROVIDER type ref to /IWBEP/IF_MGW_ENTRY_PROVIDER optional
    exporting
      !ER_ENTITY type ZCL_ZSD_ORDER_MPC=>TS_VBAP
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods VBAPSET_DELETE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
  ...
  methods VBAPSET_GET_ENTITYSET
    importing
      ...
      !IO_TECH_REQUEST_CONTEXT type ref to /IWBEP/IF_MGW_REQ_ENTITYSET optional
    exporting
      !ET_ENTITYSET type ZCL_ZSD_ORDER_MPC=>TT_VBAP
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods VBAPSET_UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
...
CLASS ZCL_ZSD_ORDER_DPC IMPLEMENTATION.


  method /IWBEP/IF_MGW_APPL_SRV_RUNTIME~CREATE_ENTITY.
*&----------------------------------------------------------------------------------------------*
*&  Include           /IWBEP/DPC_TEMP_CRT_ENTITY_BASE
*&* This class has been generated on 05/20/2020 07:42:00 in client 100
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_ZSD_ORDER_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA vbapset_create_entity TYPE zcl_zsd_order_mpc=>ts_vbap.
 DATA ht001wset_create_entity TYPE zcl_zsd_order_mpc=>ts_ht001w.
...
*     Call the entity set generated method
    vbapset_create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
       	 ...
       	 IMPORTING er_entity          = vbapset_create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
        is_data = vbapset_create_entity
      CHANGING
        cr_data = er_entity
   ).
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA vbapset_get_entity TYPE zcl_zsd_order_mpc=>ts_vbap.
 DATA ht001wset_get_entity TYPE zcl_zsd_order_mpc=>ts_ht001w.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
          vbapset_get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = vbapset_get_entity
                         es_response_context = es_response_context
          ).

        IF vbapset_get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = vbapset_get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*&*   generated methods inside the DPC provider subclass - ZCL_ZSD_ORDER_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA vbapset_get_entityset TYPE zcl_zsd_order_mpc=>tt_vbap.
 DATA ht001wset_get_entityset TYPE zcl_zsd_order_mpc=>tt_ht001w.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
      vbapset_get_entityset(
        EXPORTING
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = vbapset_get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = vbapset_get_entityset
        CHANGING
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA vbapset_update_entity TYPE zcl_zsd_order_mpc=>ts_vbap.
 DATA ht001wset_update_entity TYPE zcl_zsd_order_mpc=>ts_ht001w.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
*     Call the entity set generated method
          vbapset_update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = vbapset_update_entity
          ).
       IF vbapset_update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = vbapset_update_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
